...
export default function EditFood({popapPizzaRef}){
	...
	const addPizza = () =>{
		...
		dispatch(toggleElement('handlePopapPizza'));
	...
	console.log(pizzaData.isNew);
	console.log(pizzaData.isHit);
	return (
			<div onClick={(e)=>{
				if(e.target.className == 'styles_popap__edit__GOdN0'){
					dispatch(toggleElement('handlePopapPizza'));
				}}} className={s.popap__edit}>
				<div ref={popapPizzaRef} className={s.container__popap_edit}>
					<div className={s.popap__edit_img}>
						<img src={pizzaData.urlImg}/>
					</div>
					<div className={s.popap__edit_text}>
						<div className={s.popap__edit_top}>
							...
							<div className={s.popap__edit_info}>
								<button onMouseEnter={()=>setDescriptionIndicator(!descriptionIndicator)} onMouseLeave={()=>setDescriptionIndicator(!descriptionIndicator)} className={s.popap__info_ico}>
						...
						<div className={s.popap__edit_total}>
							...
							<button onClick={addPizza} className={s.popap__edit_add}>Добавить</button>
						</div>
					</div>
				</div>
			</div>
